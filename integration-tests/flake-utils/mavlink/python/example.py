...
Generated from: ('example.xml,example2.xml',)
...
MAVLINK_MSG_ID_RALLY_FETCH_POINT2 = 177
...
class MAVLink_rally_fetch_point_message(MAVLink_message):
    ...
    def __init__(self, target_system: int, target_component: int, idx: int):
        MAVLink_message.__init__(self, MAVLink_rally_fetch_point_message.id, MAVLink_rally_fetch_point_message.msgname)
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.target_system, self.target_component, self.idx), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_rally_fetch_point_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_rally_fetch_point2_message(MAVLink_message):
    """
    Request a current rally point from MAV. MAV should respond with a
    RALLY_POINT message. MAV should not respond if the request is
    invalid.
    ...
    id = MAVLINK_MSG_ID_RALLY_FETCH_POINT2
    msgname = "RALLY_FETCH_POINT2"
    fieldnames = ["target_system", "target_component", "idx"]
    ordered_fieldnames = ["target_system", "target_component", "idx"]
    fieldtypes = ["uint8_t", "uint8_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    ...
    def __init__(self, target_system: int, target_component: int, idx: int):
        MAVLink_message.__init__(self, MAVLink_rally_fetch_point2_message.id, MAVLink_rally_fetch_point2_message.msgname)
        self._fieldnames = MAVLink_rally_fetch_point2_message.fieldnames
        self._instance_field = MAVLink_rally_fetch_point2_message.instance_field
        self._instance_offset = MAVLink_rally_fetch_point2_message.instance_offset
        self.target_system = target_system
        self.target_component = target_component
        self.idx = idx
...
# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_rally_fetch_point2_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_RALLY_FETCH_POINT: MAVLink_rally_fetch_point_message,
    MAVLINK_MSG_ID_RALLY_FETCH_POINT2: MAVLink_rally_fetch_point2_message,
}
...
class MAVLink(object):
    ...
    def rally_fetch_point2_encode(self, target_system: int, target_component: int, idx: int) -> MAVLink_rally_fetch_point2_message:
        """
        Request a current rally point from MAV. MAV should respond with a
        RALLY_POINT message. MAV should not respond if the request is
        invalid.
        ...
        """
        return MAVLink_rally_fetch_point2_message(target_system, target_component, idx)

    def rally_fetch_point2_send(self, target_system: int, target_component: int, idx: int, force_mavlink1: bool = False) -> None:
        """
        Request a current rally point from MAV. MAV should respond with a
        RALLY_POINT message. MAV should not respond if the request is
        invalid.
        ...
        """
        self.send(self.rally_fetch_point2_encode(target_system, target_component, idx), force_mavlink1=force_mavlink1)
