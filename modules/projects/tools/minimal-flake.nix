...
let
  ...
  readme = builtins.toFile "README.md" ''
    ...
    ```nix
    inputs.${config.name}.url = "insert-url-here?dir=minimal";
    ```
  ...
  config = lib.mkIf cfg.enable {
    tools = {
      nixago.requests = lib.mkIf cfg.enable [
        {
          data = flakeNix;
          output = "minimal/flake.nix";
          hook.mode = "copy";
        }
        {
          data = flakeLock;
          output = "minimal/flake.lock";
          hook.mode = "copy";
        }
        {
          data = readme;
          output = "minimal/README.md";
          hook.mode = "copy";
        }
      ];
      treefmt.config.excludes = [ "minimal/*" ];
    };
  };
}
