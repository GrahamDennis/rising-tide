...
{injector, lib, inputs, risingTideLib, ... }:
let inherit (lib) types; in
# project tools context
{ config, pkgs, relativePaths, system, ... }: let cfg = config.nixago; in {
  options = {
  ...
  config = {
    nativeCheckInputs = lib.mkIf (cfg.requests != []) [
      (let bashSafeName = risingTideLib.sanitizeBashIdentifier "project${relativePaths.toRoot}SetupHook"; in pkgs.makeSetupHook {
